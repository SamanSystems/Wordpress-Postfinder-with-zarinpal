	...
	class PFGetSFields{
			...
			function GSF($params = array()){
				...
				switch($ftype){	
					...
					case 'selectp'://Select Box Post Types
			...
			function PFGetDefaultFieldValuesofThis($slug,$field,$exclude=0){
				...
				switch($field){
					...
					case 'posttax_group':
						$this_field_arr = array(
								'fid' => 'setupsearchfields_'.$slug.'_posttax_group',
								'ftitle' => esc_html__('Group Items','pointfindert2d'),
								'ftype' => 'switch',
								'fdesc' => '<strong>'.esc_html__('OPTIONAL : ','pointfindert2d').'</strong>'.esc_html__('If it is enabled, items in the select box will be grouped.','pointfindert2d'),
								'fdata' => array('required'=>'setupsearchfields_'.$slug.'_posttax', 'condition' => 'not_empty_and', 'required_condition_text' => '','on'=>esc_html__('Enable','pointfindert2d'),'off'=>esc_html__('Disable','pointfindert2d'))
						);
						return $this->GSF($this_field_arr);
						break;

					case 'posttax_group_ex':
						$this_field_arr = array(
								'fid' => 'setupsearchfields_'.$slug.'_posttax_group_ex',
								'ftitle' => esc_html__('Show All Option','pointfindert2d'),
								'ftype' => 'switch',
								'fdesc' => '<strong>'.esc_html__('OPTIONAL : ','pointfindert2d').'</strong>'.esc_html__('If it is enabled, you will see the main group name into the list with select all option.','pointfindert2d'),
								'fdata' => array('default'=>1,'required'=>'setupsearchfields_'.$slug.'_posttax_group', 'condition' => '=', 'required_condition_text' => '1','on'=>esc_html__('Enable','pointfindert2d'),'off'=>esc_html__('Disable','pointfindert2d'))
						);
						return $this->GSF($this_field_arr);
						break;
					...
					*/
					case 'autocmplete':
						$this_field_arr = array(
								...
								'fdata' => array('default'=>1,'on'=>esc_html__('Enable','pointfindert2d'),'off'=>esc_html__('Disable','pointfindert2d'),'required'=>'setupsearchfields_'.$slug.'_target_target','condition'=> 'contains', 'required_condition_text' => array('address','title'))
						);
						return $this->GSF($this_field_arr);
						break;
				...
				if($slug != '' && $field != '' && $title != ''){
					...
					switch($field){
						case '1':
							...
							$multiple_val[] = $this->PFGetDefaultFieldValuesofThis($slug,'posttax_group');
							$multiple_val[] = $this->PFGetDefaultFieldValuesofThis($slug,'posttax_group_ex');
							...
							//$multiple_val[] = $this->PFGetDefaultFieldValuesofThis($slug,'indentstart','rvalues');
							...
							//$multiple_val[] = $this->PFGetDefaultFieldValuesofThis($slug,'indentend','rvalues');
							$multiple_val[] = $this->PFGetDefaultFieldValuesofThis($slug,'validation_required');
							//$multiple_val[] = $this->PFGetDefaultFieldValuesofThis($slug,'indentstart','val');
							$multiple_val[] = $this->PFGetDefaultFieldValuesofThis($slug,'message');
							//$multiple_val[] = $this->PFGetDefaultFieldValuesofThis($slug,'indentend','val');
