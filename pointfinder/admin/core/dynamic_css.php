...
/* Start: General Settings (Border etc...) */
	...
	$csstext .= '#pfsearch-draggable .slider-input,.golden-forms input, .golden-forms button, .golden-forms select, .golden-forms select optgroup, .golden-forms textarea, .wpcf7 input, .wpcf7 button, .wpcf7 select, .wpcf7 textarea,.select2-results .select2-result-label{font-size:'.$tcustomizer_typographyh_main.';}';
	$csstext .= '#pfuaprofileform .select,#pfuaprofileform .button,.pfmu-payment-area .select,.pfmu-payment-area .select:hover,.pfmu-itemlisting-inner .pfmu-userbuttonlist-item .button,.widget_tag_cloud a,.golden-forms #commentform .button,.ui-tabgroup >.ui-tabs >[class^="ui-tab"],.pfmu-itemlisting-inner .pfmu-userbuttonlist-item .button:hover{border:1px solid '.$setup30_dashboard_styles_bodyborder.'}';
...
/* Start: Menu Bar configuration */
	...
	*/
	...
	/*Main Menu Settings*/
		...
		$csstext .= '#pf-primary-nav li.current_page_item > a, #pf-primary-nav .pfnavmenu li > a:hover{ border-bottom:2px solid '.$setup18_headerbarsettings_menulinecolor.';}';
	...
	/*Logo Settings*/
		$csstext .= '.wpf-header .pf-logo-container{margin:'.$setup18_headerbarsettings_padding.' 0;height: '.$setup17_logosettings_sitelogo_height_number.'px;}';
...
if ( ! $wp_filesystem->is_dir( $upload_dir ) ) {
	if ( ! $wp_filesystem->mkdir( $upload_dir, 0755 ) ) {
		...
		function pointfinder_css_system_status2() {
        	...
        	echo '<h3>'.__('Point Finder: CSS Folder System Error','pointfindert2d').'</h3>';
			echo 'Error Code: '.$wp_filesystem->errors->get_error_code();
			echo '<br/>Error Message: '.__( 'Folder can not create.', 'pointfindert2d' );
...
if ( ! $wp_filesystem->put_contents($filename, $csstext, FS_CHMOD_FILE) ) {
	if ( is_wp_error( $wp_filesystem->errors ) && $wp_filesystem->errors->get_error_code() ) {
		...
		function pointfinder_css_system_status() {
			global $wp_filesystem;
			echo '<div class="error"><p>'; 
        	echo '<h3>'.__('Point Finder: CSS File System Error','pointfindert2d').'</h3>';
			echo 'Error Code: '.$wp_filesystem->errors->get_error_code();
			echo '<br/>Error Message: '.__( 'Something went wrong: dynamic_css.css could not be created.', 'pointfindert2d' );
			echo '<br/>Error Detail: '.$wp_filesystem->errors->get_error_message();
			echo "</p></div>";
		}
		...
		function pointfinder_css_system_status() {
			global $wp_filesystem;
			echo '<div class="error"><p>'; 
        	echo '<h3>'.__('Point Finder: CSS SFile ystem Error','pointfindert2d').'</h3>';
			echo 'Error Code: '.$wp_filesystem->errors->get_error_code();
			echo '<br/>Error Message: '.__( 'dynamic_css.css could not be created. Connection error.', 'pointfindert2d' );
			echo "</p></div>";
		}
		...
		function pointfinder_css_system_status() {
        	...
        	echo '<h3>'.__('Point Finder: CSS File System Error','pointfindert2d').'</h3>';
			echo 'Error Code: '.$wp_filesystem->errors->get_error_code();
			echo '<br/>Error Message: '.__( 'dynamic_css.css could not be created. Cannot write dynamic_css css to "' . $filename . '".', 'pointfindert2d' );
			echo "</p></div>";
		}
	} else {
		add_action('admin_notices', 'pointfinder_css_system_status');
		function pointfinder_css_system_status() {
        	...
        	echo '<h3>'.__('Point Finder: CSS File System Error','pointfindert2d').'</h3>';
			echo 'Error Code: '.$wp_filesystem->errors->get_error_code();
			echo '<br/>Error Message: '.__( 'dynamic_css.css could not be created. Problem with access.', 'pointfindert2d' );
			echo "</p></div>";
		}
	}
