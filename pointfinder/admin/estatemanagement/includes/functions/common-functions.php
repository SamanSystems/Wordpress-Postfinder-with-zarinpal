...
**/
	...
	function pf_get_item_term_id($the_post_id){
		...
		if (!empty($the_post_id)) {
			$item_term_get = get_the_terms( $the_post_id, 'pointfinderltypes' );
			foreach ($item_term_get as $key => $value) {
				
				if (!empty($value->parent)) {
					$item_term = $value->parent;
				}else{
					$item_term = $value->term_id;
				}
		        ...
		        if (is_array($array[$key])) {
		        	...
		        	if(!in_array($key, $exclude_list)){
		            	$array[$key] = call_user_func($callback, $array[$key]);
		           		...
		           		$array[$key] = esc_attr(wp_kses_post($array[$key]));
		           } 
		        }
		    }
		    return $array;
		...
		if($pfstart == true){
			...
			if($setup22_searchresults_hide_lt == 0){
				...
				if($pfitemtext != ''){
					...
					$term_names = GetPFTermInfo($id,'pointfinderltypes');
	    ...
	    if($_page_titlebararea == 1){
        	...
        	echo '
        		<div class="pf-container">
        			<div class="pf-row">
        				<div class="col-lg-12"'.$_page_custom_css.'">
	...
	function PFGetDefaultPageHeader($params = array()){
		...
		if(empty($params['itemname'])){
		    ...
		    if (get_the_id() == $setup4_membersettings_dashboard) {
		    	...
		    	switch ($ua_action) {
					case 'profile':
						$titletext = PFSAIssetControl('setup29_dashboard_contents_profile_page_menuname','','Profile');
					break;
					case 'favorites':
						$titletext = PFSAIssetControl('setup29_dashboard_contents_favs_page_menuname','','My Favorites');
					break;
					case 'newitem':
						$titletext = PFSAIssetControl('setup29_dashboard_contents_submit_page_menuname','','Submit New Item');
					break;
					case 'edititem':
						$titletext = PFSAIssetControl('setup29_dashboard_contents_submit_page_titlee','','Edit Item');
					break;
					case 'reviews':
						$titletext = PFSAIssetControl('setup29_dashboard_contents_rev_page_menuname','','My Reviews');
					break;
					case 'myitems':
						$titletext = PFSAIssetControl('setup29_dashboard_contents_my_page_menuname','','My Items');
					break;
					default:
