...
function PFGetAgentPageCol1($author_id){
			...
			$tabinside .= '<div id="pf-itempage-sidebarinfo">';
				...
				$tabinside .= '<div class="pf-itempage-sidebarinfo-userdetails pfpos2">
					...
					if(!empty($user_description)){
						$tabinside .= '<li class="pf-itempage-sidebarinfo-eldesc pf-itempage-sidebarinfo-elitem">'.get_the_content().'</li>';
					}
	...
	echo do_shortcode('[pf_itemgrid2 orderby="title" sortby="ASC" items="'.$setup42_authorpagedetails_defaultppptype.'" cols="3" grid_layout_mode="'.$setup42_authorpagedetails_grid_layout_mode.'" filters="true" itemboxbg="'.$setup42_authorpagedetails_background2.'" agentmode="1" author="'.$author_id.'"]' );
	echo '</div>';
	echo '</div>';
	echo '</div>';
}
