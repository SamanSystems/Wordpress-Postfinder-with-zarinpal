...
function PFGetItemPageCol1(){
	...
	if ($setup3_modulessetup_headersection != 0) {
		echo '<div class="pf-item-title-bar"><span class="pf-item-title-text" itemprop="name">'.get_the_title().'</span> <span class="pf-item-subtitle"> '.esc_html(get_post_meta( get_the_id(), 'webbupointfinder_items_address', true )).'</span></div><div class="pf-item-extitlebar"><div class="pf-itemdetail-pdate">'.esc_html__('Posted on','pointfindert2d').' '.get_the_time('F j, Y').' / <i class="pfadmicon-glyph-729"></i> '.$item_old_count.' '.$verified_badge_text.'</div></div>'; 
	}else{
		echo '<div class="pf-item-title-bar">'.$verified_badge_text.'<div class="pf-itemdetail-pdate">'.esc_html__('Posted on','pointfindert2d').' '.get_the_time('F j, Y').' / <strong><i class="pfadmicon-glyph-729"></i> '.$item_old_count.'</strong></div></div>'; 
	}
					...
					**/
						...
						if((!empty($images) || !empty($featured_image[0]))){
							...
							$tabinside .= '<section role="itempagegallery" class="pf-itempage-gallery pf-itempage-elements">';
								...
								$output = $output2 = '';
								$kl = 0;
								if (PFSAIssetControl('setup42_itempagedetails_featuredimage','','1') == 1) {
								...
								if(!empty($images)){
									foreach ( $images as $image )
									{
									    ...
									    $output .= "<li class='item'><a href='{$image['full_url']}' title='".get_the_title()."' rel='prettyPhoto'><img src='{$image['full_url']}' alt='{$image['alt']}' /></a></li>";
										...
										if($user_photo != ''){
											...
											if ($setup42_itempagedetails_contact_form == 1) {
												...
												$val1 .= ' '.get_user_meta($user_id, 'last_name', true);
													...
													$namefield = '<section'.$exstyles.'><label class="lbl-ui"><input type="hidden" name="name" class="input" placeholder="" value="'.$val1.'" /></label></section>';
													$emailfield = '<section'.$exstyles.'><label class="lbl-ui"><input type="hidden" name="email" class="input" placeholder="" value="'.$val2.'"/></label></section>  ';
												}else{
												...
												<div class="golden-forms">
													...
													<form id="pf-ajax-enquiry-form">
													    ...
													    <div class="row"'.$rowval.'><div class="col6 first">'.$namefield.'</div><div class="col6 last">'.$emailfield.'</div></div>
