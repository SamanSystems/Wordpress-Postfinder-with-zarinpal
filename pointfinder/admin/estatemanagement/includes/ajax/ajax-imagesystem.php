		...
		if (get_current_user_id() == $post_author) {
			...
			if ($id != '') {
				...
				if (PFControlEmptyArr($images_of_thispost) || !empty($imageID_of_featured)) {
					...
					$output_images .= '<div class="pf-itemimage-delete"><a class="pf-delete-standartimg" data-pfimgno="'.$imageID_of_featured.'" data-pfpid="'.$id.'" data-pffeatured="yes">'.esc_html__('Remove', 'pointfindert2d').'</a></div><div class="pfitemedit-featured"><div>'.esc_html__('Cover Photo', 'pointfindert2d').'</div></div>';
					$output_images .= '</div>';
					$output_images .= '</li>';
					...
					foreach ($images_of_thispost as $image_number) {
						...
						$output_images .= '<div class="pf-itemimage-container">';
						$output_images .= '<img src="'.aq_resize($image_src[0],90,90,true).'">';
						$output_images .= '<div class="pf-itemimage-delete"><a class="pf-delete-standartimg" data-pfimgno="'.$image_number.'" data-pfpid="'.$id.'" data-pffeatured="no">'.esc_html__( 'Remove', 'pointfindert2d' ).'</a></div><div class="pfitemedit-featured"><a class="pf-change-standartimg" data-pfimgno="'.$image_number.'" data-pfpid="'.$id.'" title="'.esc_html__('You can change your cover photo by clicking here', 'pointfindert2d').'">'.esc_html__('Set as Cover', 'pointfindert2d').'</a></div>';
						$output_images .= '</div>';
						$output_images .= '</li>';
					}
