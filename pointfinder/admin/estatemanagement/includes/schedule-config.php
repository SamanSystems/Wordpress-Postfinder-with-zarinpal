	...
	*/
	...
	if ($setup31_userpayments_pendinglimit > 0) {
		...
		$results = $wpdb->get_results( $wpdb->prepare( 
			"SELECT p.ID, p.post_author, pm.meta_value, p.post_date FROM $wpdb->posts as p INNER JOIN $wpdb->postmeta as pm  ON ( p.ID = pm.post_id ) WHERE p.post_type = %s and p.post_status = %s and p.post_date <= %s and pm.meta_key = %s", 
			'pointfinderorders',
			'pendingpayment',
			$exptime,
