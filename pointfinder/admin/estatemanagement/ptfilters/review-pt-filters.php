...
***********************************************************************************************************************************/
	...
	**/
	...
	function pf_rev_remove_unused_links($actions, $page_object)
	{   
	    global $post;
	    if($page_object->post_type == 'pointfinderreviews'){
	        unset($actions['view']);
	        unset($actions['inline hide-if-no-js']);
	        unset($actions['edit_as_new_draft']);
	        unset( $actions['trash'] );
	        $actions['trash'] = "<a class='submitdelete' title='" . esc_attr(esc_html__('Delete this item permanently','pointfindert2d')) . "' href='" . get_delete_post_link($post->ID, '', true) . "'>" . esc_html__('Delete Permanently','pointfindert2d') . "</a>";
	    }
	    return $actions;
	}
	add_filter('page_row_actions', 'pf_rev_remove_unused_links', 10, 2);

	add_action('admin_head','pf_page_row_actions_01');

	function pf_page_row_actions_01(){
	    $screen = get_current_screen();
	    if($screen->post_type == 'pointfinderreviews'){
	        global $submenu;
			unset($submenu['edit.php?post_type=pointfinderreviews'][10]); 
	        echo '<style type="text/css">#titlediv{margin-bottom: 10px;}#edit-slug-box{display: none;}#favorite-actions {display:none;}.add-new-h2{display:none;}.tablenav{display:none;}</style>';
	    }
	};



	add_filter( 'manage_edit-pointfinderreviews_columns', 'edit_reviews_columns' );
	...
	add_filter( 'manage_edit-pointfinderreviews_sortable_columns', 'reviews_sortable_columns' );

	function reviews_sortable_columns( $columns ) {
	...
	add_action( 'manage_pointfinderreviews_posts_custom_column', 'manage_reviews_columns', 10, 2 );
