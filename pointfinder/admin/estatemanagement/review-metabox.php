...
**/
	function pointfinder_reviews_meta_box_revinfo( $post ) {
		...
		$output = '<ul>';
			$output .= '<li><span class="rev-cr-title">'.esc_html__('Item').' : </span><a href="'.get_edit_post_link($itemid).'" target="_blank">'.get_the_title($itemid).'('.$itemid.')</a></li>';
			...
			if (PFcheck_postmeta_exist('webbupointfinder_review_userid',$post->ID)) {
				...
				$output .= '<li><span class="rev-cr-title">'.esc_html__('Reviewer').' : </span><a href="'.get_edit_user_link($userid).'" target="_blank">'.get_the_title($post->ID).'</a></li>';
			}else{
				$output .= '<li><span class="rev-cr-title">'.esc_html__('Reviewer').' : </span>'.get_the_title($post->ID).'</li>';
			}
			
			$output .= '<li><span class="rev-cr-title">'.esc_html__('Email').' : </span>'.$email.'</li>';
	...
	<div class="submitbox" id="submitpost">
		...
		<div id="misc-publishing-actions">
			...
			<div class="misc-pub-section misc-pub-revisions">
				...
				?>
				<a class="hide-if-no-js" href="<?php echo esc_url( get_edit_post_link( $args['args']['revision_id'] ) ); ?>"><span aria-hidden="true"><?php _ex( 'Browse', 'revisions' ); ?></span> <span class="screen-reader-text"><?php esc_html_e( 'Browse revisions' ,'pointfindert2d'); ?></span></a>
			</div>
			<?php };
